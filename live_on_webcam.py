...
def create_matplotlib_window():
    ...
    fig.set_size_inches(19.20, 10.80 + 0.02)  # 1 inch is 100 pixels
...
parser.add_argument('--size', type=int, default=256, help='size of the data (squared assumed)') # Default = 256
    ...
    def __init__(self):
        if activate_source_sender_result_receiver is False:
            ...
            for key_style_model_folderpath, values_style_model_folderpath in models_statedict_folderpaths_per_style.items():
                ...
                self.netG_A2B_dict_for_all_styles[key_style_model_folderpath] = Generator(opt.input_nc, opt.output_nc).to(self.device)
    ...
    def start_network_loop(self):
        ...
        while True:
            if activate_source_sender_result_receiver is not True:
                ...
                if (self.current_selected_style_type_or_name == "random" or self.current_selected_style_type_or_name is None
            ...
            if need_to_update_generated_image is True:
                ...
                if activate_source_receiver_result_sender is True:
                    plt.savefig(os.path.join(self.current_dir_path, "temp", "image_generated_to_send.jpg"))
